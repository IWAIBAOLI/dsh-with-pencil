...
import { clearSessionFile, setSessionFile } from './session-file.js'

/** Set the binding's current file and record it as the session's working file. */
function setCurrentFile(binding, target) {
  binding.currentFile = target
  setSessionFile(binding.sessionId, target)
}
...
export function registerCanvasHost({ ctx, sub, mcpBin, headless }) {
  ...
  function releaseBinding(binding, reason) {
    ...
    clearSessionFile(binding.sessionId)
  }
  ...
  async function handleBind(req, res) {
    ...
    setSessionFile(sessionId, currentFile)
    res.writeHead(200, { 'Content-Type': 'application/json' })
  ...
  async function handleRename(req, res) {
    ...
    if (!body || !body.file || !body.name) { res.writeHead(400); res.end('file and name are required'); return }
    let target, next
    try {
      target = insideWorkspace(binding, body.file)
      next = insideWorkspace(binding, body.name)
    } catch (error) { res.writeHead(403); res.end(error.message); return }
    if (path.extname(target).toLowerCase() !== '.pen' || path.extname(next).toLowerCase() !== '.pen') {
      res.writeHead(400); res.end('canvas files must end in .pen'); return
    }
    if (target === next) { res.writeHead(409); res.end('the new name is the same as the current name'); return }
    try {
      await enqueueCanvas(binding, async () => {
        try { await fsp.access(target) }
        catch (error) {
          if (error && error.code === 'ENOENT') { const e = new Error('file does not exist: ' + body.file); e.status = 404; throw e }
          throw error
        }
        let exists = false
        try { await fsp.access(next); exists = true }
        catch (error) { if (!error || error.code !== 'ENOENT') throw error }
        if (exists) throw new Error('a file with that name already exists')
        if (binding.currentFile === target && binding.initialized && binding.loadedFile === target) await saveCanvas(binding)
        await fsp.rename(target, next)
        if (binding.currentFile === target) {
          resources.stopDocumentWatcher(binding)
          setCurrentFile(binding, next)
          binding.loadedFile = null
          binding.conflict = null
          binding.saveError = null
          binding.dirty = false
      ...
      res.writeHead(error && error.status || 409); res.end(error && error.message ? error.message : String(error)); return
    }
    res.writeHead(200, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ ok: true, file: next }))
  }
  async function handleDelete(req, res) {
    ...
    if (!body || !body.file) { res.writeHead(400); res.end('file is required'); return }
    let target
    try { target = insideWorkspace(binding, body.file) }
    catch (error) { res.writeHead(403); res.end(error.message); return }
    if (path.extname(target).toLowerCase() !== '.pen') {
    ...
    try {
      await enqueueCanvas(binding, async () => {
        ...
        if (binding.currentFile === target) {
          resources.stopDocumentWatcher(binding)
          const remaining = await listPenFiles(binding.workspace)
          nextFile = remaining.length ? remaining[0] : null
          const fallback = nextFile ? resolveWorkspacePath(binding.workspace, nextFile, { extension: '.pen', label: 'file' }) : defaultFile(binding.workspace)
          setCurrentFile(binding, fallback)
          binding.loadedFile = null
          binding.conflict = null
          binding.saveError = null
          binding.dirty = false
          binding.autosaveAfter = Infinity
          await queueCurrentFile(binding)
          nextFile = path.relative(binding.workspace, fallback).split(path.sep).join('/')
        }
      })
    } catch (error) {
      res.writeHead(error && error.status || 409); res.end(error && error.message ? error.message : String(error)); return
  ...
  async function handleReveal(req, res) {
    ...
    try {
      const handle = sub.spawn({
        argv, cwd: binding.workspace, env: {},
    ...
    if (msg.type === 'notification') {
      if (msg.method === 'initialized') {
        ...
        try {
          ...
          setCurrentFile(binding, insideWorkspace(binding, uri))
          binding.loadedFile = null
          binding.conflict = null
          binding.saveError = null
          binding.autosaveAfter = Infinity
  ...
  const canvasBridge = {
    has(exec) {
      // A connecting canvas is worth waiting for (it just opened); an
      // initialized and recently-seen one is usable. Only a stale binding
      // (hidden for a while) falls back to the headless engine.
      const binding = bindingForExec(exec)
      if (!binding) return false
      if (!binding.initialized) return true
      return Date.now() - binding.lastSeen < 30000
    },
    ...
    // Whole-document export for internal screenshot composition: every
    // top-level node rendered through the live editor IPC (the official
    // webview renderer), written as <nodeId>.png into a workspace-relative
    // outputDir. This is deliberately NOT the export_nodes tool path —
    // "document" is not a real node id and the tool's outputDir is a
    // workspace-relative user-facing directory.
    async documentExport(exec, { filePath, outputDir, format = 'png', scale = 2, signal } = {}) {
      const binding = bindingForExec(exec)
      if (!binding) return { ok: false, text: 'No live canvas is bound to this conversation.' }
      return enqueueCanvas(binding, async () => {
        try {
          await waitForCanvasReady(binding)
          await selectCanvasFile(binding, filePath)
          if (binding.conflict) throw new Error('resolve the external file conflict before exporting')
          await saveCanvas(binding)
          const targetDir = insideWorkspace(binding, outputDir)
          await fsp.mkdir(targetDir, { recursive: true })
          const treeResponse = await transport.request(binding, 'batch-get', { readDepth: 0 }, 30000, signal)
          const tree = treeResponse && treeResponse.success === false
            ? null
            : treeResponse && Object.prototype.hasOwnProperty.call(treeResponse, 'result') ? treeResponse.result : treeResponse
          ...
          if (!nodes.length) return { ok: false, text: 'the canvas is empty; add a frame before exporting' }
          const files = []
          for (const node of nodes) {
            const response = await transport.request(binding, 'export-nodes', {
              nodeIds: [node.id], format, scale,
            }, 120000, signal)
            if (response && response.success === false) {
              return { ok: false, text: 'Canvas error: ' + String(response.error || 'export failed').slice(0, 4000) }
            }
            const exported = response && Object.prototype.hasOwnProperty.call(response, 'result') ? response.result : response
            const image = exported && Array.isArray(exported.images) ? exported.images[0] : undefined
            if (!image || !image.image) return { ok: false, text: 'the editor returned no export data for ' + node.id }
            const target = path.join(targetDir, node.id + '.png')
            await writeFileAtomic(target, Buffer.from(String(image.image), 'base64'))
            files.push(target)
          }
          return { ok: true, files, nodes }
        } catch (err) {
          return { ok: false, text: 'Live canvas document export failed: ' + (err && err.message ? err.message : String(err)) }
        }
      })
    },
    async run(tool, args, opts, fileArg) {
      ...
      return enqueueCanvas(binding, async () => {
        try {
          ...
          if (method === 'export-nodes') {
            // Webview export is the reliable renderer (the headless engine
            // times out or fails on complex nodes). "document" is handled by
            // the existing manual exporter (selection or all top-level nodes);
            // explicit node ids export one file per node.
            await saveCanvas(binding)
            let nodeIds = Array.isArray(payload.nodeIds) ? payload.nodeIds.map(String) : []
            if (nodeIds.length === 0 || nodeIds.includes('document')) {
              const format = payload.format === 'pdf' ? 'pdf' : 'png'
              const result = await exporter.run(binding, format)
              if (!result.ok) return { ok: false, text: 'Canvas export failed: ' + String(result.error || 'unknown error') }
              if (!args.outputDir) return { ok: false, text: 'outputDir is required' }
              const outputDir = insideWorkspace(binding, args.outputDir)
              await fsp.mkdir(outputDir, { recursive: true })
              const moved = []
              for (const relative of result.files) {
                const source = path.join(binding.workspace, relative)
                const target = path.join(outputDir, path.basename(source))
                await fsp.rename(source, target)
                moved.push(target)
              }
              return { ok: true, mode: 'canvas', text: 'Exported ' + moved.length + ' file(s):\n' + moved.join('\n') }
            }
            if (!args.outputDir) return { ok: false, text: 'outputDir is required' }
            const outputDir = insideWorkspace(binding, args.outputDir)
            await fsp.mkdir(outputDir, { recursive: true })
            ...
            for (const nodeId of nodeIds) {
              const exportResponse = await transport.request(binding, 'export-nodes', {
                nodeIds: [nodeId],
                format: payload.format || 'png',
                scale: payload.scale === undefined ? 2 : payload.scale,
              }, 120000, opts.signal)
              if (exportResponse && exportResponse.success === false) {
                return { ok: false, text: 'Canvas error: ' + String(exportResponse.error || 'export failed').slice(0, 4000) }
              }
              const exported = exportResponse && Object.prototype.hasOwnProperty.call(exportResponse, 'result') ? exportResponse.result : exportResponse
              const images = exported && Array.isArray(exported.images) ? exported.images : []
              const image = images[0]
              if (!image || !image.image) return { ok: false, text: 'the editor returned no export data for ' + nodeId }
              const name = (image.nodeId || 'export') + (image.mimeType === 'application/pdf' ? '.pdf' : '.png')
              const target = path.join(outputDir, name)
              await writeFileAtomic(target, Buffer.from(String(image.image || ''), 'base64'))
              files.push(target)
            }
            return { ok: true, mode: 'canvas', text: 'Exported ' + files.length + ' file(s):\n' + files.join('\n') }
          }
          ...
          if (tool === 'batch_get') {
            ...
            return { ok: true, mode: 'canvas', text: JSON.stringify(nodes).slice(0, 8000) }
          }
          ...
          return {
            ok: true,
            mode: 'canvas',
            text: text.slice(0, tool === 'get_app_state' ? 12000 : 8000),
  ...
  routeDisposers.push(webServer.register({ kind: 'exact', path: '/pen-host/rename', handler: handleRename }))
  routeDisposers.push(webServer.register({ kind: 'exact', path: '/pen-host/delete', handler: handleDelete }))

...
export function createHeadlessRuntime({ ctx, sub, penBin, mcpBin, baseEnv, workspaceForExec }) {
  ...
  async function runMcpNow(tool, args, opts) {
    ...
    // Keep every document-sensitive operation on the live webview while a
    // conversation canvas is active. Starting a second headless document for
    // reads/screenshots can observe an empty or stale scene even after the
    // live canvas has durably saved the requested file.
    const canvasPreferred = tool === 'execute' || tool === 'get_app_state'
      || tool === 'batch_get' || tool === 'export_nodes'
  ...
  function runMcp(tool, args, opts) {
    ...
    if ((tool === 'execute' || tool === 'get_app_state' || tool === 'batch_get' || tool === 'export_nodes')
      && canvasBridge && canvasBridge.has(opts.exec) && canvasBridge.supports(tool)) {

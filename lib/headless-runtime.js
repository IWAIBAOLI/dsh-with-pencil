...
export function createHeadlessRuntime({ ctx, sub, penBin, mcpBin, baseEnv, workspaceForExec }) {
  ...
  function adaptArguments(actual, args, spec, fileArg) {
    ...
    if (fileArg && required.includes('filePath') && !adapted.filePath) adapted.filePath = fileArg
    return adapted
  }
  ...
  async function runMcpNow(tool, args, opts) {
    ...
    if (fileArg) {
      try { await ensureEngine(fileArg, opts.exec); appName = 'cli' }
    ...
    try {
      ...
      return { ok: !(response && response.result && response.result.isError), text: text.slice(0, 8000), ...(imageData ? { imageData, imageMediaType } : {}) }
    } catch (error) {
  ...
  function runMcp(tool, args, opts) {
    ...
    if (canvasBridge && canvasBridge.has(opts.exec) && canvasBridge.supports(tool)) {

  ...
  function editorDirectory() {
    ...
    for (let depth = 0; depth < 5; depth += 1) {
      ...
      try {
        ...
        const spec = pkg && pkg.dependencies && pkg.dependencies['pen-dev-bridge']
        if (typeof spec === 'string' && (spec.startsWith('file:') || spec.startsWith('link:'))) {
          const bridgeDir = path.resolve(cursor, spec.slice(spec.indexOf(':') + 1))
          candidates.push(path.resolve(bridgeDir, '../pen-editor/out'))
        }

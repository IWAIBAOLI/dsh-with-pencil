...
import { createSessionImages } from './session-images.js'
...
export default {
  ...
  apply(ctx, config = {}) {
    ...
    const sessionImages = createSessionImages()
    const toolCount = registerModelTools({ ctx, attachments, sessionImages, headless, workspaceForExec, vision })
    ...
    if (typeof ctx.inject === 'function') {
      ctx.inject(['settings'], (settingsCtx) => {
        try {
          ...
          settingsCtx.effect(() => () => {
            vision.mode = config.visionMode === 'multimodal' ? 'multimodal' : 'text'
          }, 'dsh-with-pencil: settings fallback')
        } catch (error) {
          console.error('[dsh-with-pencil] pencil settings namespace registration failed:', error)
        }
      })
    }
    // Expose the namespace to the Web configuration boundary. The API proxy
    // serves settings describe/mutate ONLY for configurable-provider
    // namespaces plus a fixed allowlist; without this directory entry the
    // settings card's scope binder reports the namespace as unavailable.
    // The same mechanism is what dsh-vision-router uses for its card.
    if (ctx.llm && typeof ctx.llm.registerConfigurableProviders === 'function') {
      try {
        const directory = ctx.llm.registerConfigurableProviders([{
          provider: 'pencil',
          displayName: 'Pencil 设计插件（官方限制占位）',
          settingsNs: PENCIL_SETTINGS_NS,
          settingsPath: [],
        }])
        ctx.effect(() => directory, 'dsh-with-pencil: configurable provider directory')
      } catch (error) {
        console.warn('[dsh-with-pencil] configurable provider registration failed:', error)
      }
    }
    ...
    // Track chat image attachments (ImageAttachmentRefs) so pencil_mcp_insert_image
    // can resolve "that image from the chat" by its attachment id. This only records
    // refs seen in the model-visible pre-step messages — it never rewrites or strips
    // content.
    if (typeof ctx.on === 'function') {
      ctx.on('agent/pre-step', async (payload, next) => {
        const session = payload && payload.agent && payload.agent.session
        // Capture the raw boundary first: a vision wrapper later in the
        // waterfall may replace ImageBlocks with text descriptions/markers.
        sessionImages.record(session, (payload && payload.messages) || [])
        const decision = await next()
        // Also capture images injected by downstream plugins or tool results.
        if (decision && decision.kind !== 'reject') {
          sessionImages.record(session, decision.messages || [])
        }
        return decision
      })
      ctx.on('session/disposed', (session) => {
        sessionImages.clear(session)
      })
    }

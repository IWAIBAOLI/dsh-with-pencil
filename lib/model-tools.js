...
import { writeImageAsset, sniffImageMediaType, extensionToMediaType } from './image-assets.js'
...
export function registerModelTools({ ctx, attachments, sessionImages, headless, workspaceForExec, vision }) {
  ...
  register('pencil_mcp_insert_image',
    ...
    async (args, exec) => {
      ...
      if (!mediaType) return { ok: false, text: 'pencil_mcp_insert_image: unsupported image format (png/jpeg/webp/gif only)' }

      // 3. Write the image into images/ next to the .pen (pen.dev convention).
      const { relativeUrl } = await writeImageAsset(path.dirname(penFile), bytes, mediaType)

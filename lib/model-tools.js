...
export function registerModelTools({ ctx, attachments, sessionImages, headless, workspaceForExec, vision }) {
  ...
  register('pencil_mcp_insert_image',
    'Insert a chat image (or a local image file) onto the .pen canvas using pen.dev\'s official image-fill: it writes the image into the `images/` directory next to the .pen and adds a Frame whose `fill` is `{type:"image", url, mode}`. Pass a chat image attachment id (from an image uploaded in this conversation) or a local image file path as `image`; target the .pen via `filePath` (defaults to the current .pen) and optionally set `parentId` / `width` / `height` / `x` / `y` / `mode` (fit|fill|stretch, default fit). Returns the .pen-relative image URL and the created node.',
    {
      image: { type: 'string', required: true, description: 'Chat image attachment id, or a local image file path (workspace-relative or absolute).' },
    ...
    async (args, exec) => {
      ...
      let mediaType
      let via = 'file'
      const ref = sessionImages && sessionImages.lookup(session, String(args.image))
      ...
      if (!bytes) {
        const resolved = resolveWorkspacePath(workspace, String(args.image), { label: 'image' })
        try {

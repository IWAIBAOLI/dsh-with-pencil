...
export function registerModelTools({ ctx, attachments, headless, workspaceForExec, visionMode = 'text' }) {
  ...
  async function materializeToolResult(value, toolName) {
    ...
    const withInstruction = toolName === 'pencil_mcp_get_screenshot' && visionMode !== 'multimodal'
  ...
  // (exporter.run) when a canvas is active — it is far more reliable than the
  // headless engine for whole-document rendering. The exported node images
  // are composed back into one document view by document coordinates. If the
  ...
  async function composeDocumentShot(filePath, exec) {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pen-doc-'))
    try {
      const docExport = await runMcp('export_nodes', { filePath, nodeIds: ['document'], outputDir: tmpDir, format: 'png', scale: 2 }, { exec, signal: exec.signal })
      if (!docExport.ok) return null
    ...
    async (args, exec) => {
      ...
      if (nodeId !== 'document') {
        ...
        if (big) {
          ...
          const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pen-shot-'))
          try {

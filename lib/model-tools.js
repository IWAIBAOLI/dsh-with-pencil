...
import { getSessionFile, setSessionFile, sessionIdOf } from './session-file.js'
import { IMAGE_MEDIA_TYPES, writeImageAsset, sniffImageMediaType, extensionToMediaType } from './image-assets.js'
...
export function registerModelTools({ ctx, attachments, sessionImages, headless, workspaceForExec, vision }) {
  ...
  const PEN_FILE_PATH_DESCRIPTION =
    'Path relative to the exact session workspace (cwd); do not repeat the workspace directory name.'
  ...
  // webview is unavailable, return null so a headless-only conversation can
  // use the native renderer. Once a live canvas owns the document, fail with
  // its real diagnostic instead of silently switching to a divergent engine.
  async function composeDocumentShot(filePath, exec) {
    ...
    try {
      const docExport = await headless.canvasDocumentExport(exec, {
        filePath, outputDir: tmpDir, format: 'png', scale: 2, signal: exec.signal,
      })
      if (!docExport || !docExport.ok) {
        return { ok: false, text: 'Live canvas document screenshot failed: ' + String(docExport && docExport.text || 'no export result') }
      }
      const files = fs.readdirSync(tmpDir).filter((f) => f.toLowerCase().endsWith('.png'))
      if (!files.length) return { ok: false, text: 'Live canvas document screenshot failed: export produced no PNG files' }
      const nodes = docExport.nodes
      const tops = Array.isArray(nodes) ? nodes.filter((n) => {
        const w = Number(n && n.width)
        const h = Number(n && n.height)
        return Number.isFinite(w) && Number.isFinite(h) && w > 0 && h > 0
      }) : []
      if (!tops.length) return { ok: false, text: 'Live canvas document screenshot failed: no renderable top-level nodes' }
      if (tops.length > 20) return { ok: false, text: 'Live canvas document screenshot failed: more than 20 top-level nodes cannot be composed safely' }
      const sharpModule = await import('sharp').then((m) => m.default).catch(() => null)
      if (!sharpModule) return { ok: false, text: 'Live canvas document screenshot failed: Sharp is unavailable' }
      ...
      if (!layers.length) return { ok: false, text: 'Live canvas document screenshot failed: exported node images did not match the live node tree' }
      ...
      return { ok: false, text: 'Live canvas document screenshot failed: ' + (error && error.message ? error.message : String(error)) }
    } finally {
      try { await fsp.rm(tmpDir, { recursive: true, force: true }) } catch { /* best effort */ }
    }
  }

  async function exportNodeShot(filePath, nodeId, exec, { liveRequired = false } = {}) {
    const tmpDir = fs.mkdtempSync(path.join(workspaceForExec(exec), '.pen-shot-'))
    try {
      const exported = await retryExport(() => runMcp('export_nodes', {
        filePath, nodeIds: [nodeId], outputDir: tmpDir, format: 'png', scale: 1,
      }, { exec, signal: exec.signal }))
      if (!exported.ok) {
        return liveRequired
          ? { ok: false, text: 'Live canvas node screenshot failed: ' + exported.text }
          : null
      }
      const files = fs.readdirSync(tmpDir).filter((f) => f.toLowerCase().endsWith('.png'))
      if (!files.length) {
        return liveRequired
          ? { ok: false, text: 'Live canvas node screenshot failed: export produced no PNG file' }
          : null
      }
      const data = fs.readFileSync(path.join(tmpDir, files[0]))
      return {
        ok: true,
        text: '[screenshot: ' + Math.round(data.length / 1024) + ' KB image/png]',
  ...
  register('pencil_mcp_open', 'Open (or switch) the conversation\'s .pen file — call FIRST for any design work. With a live canvas the file loads there and edits render live; otherwise a local headless engine is used. Returns the current app state.',
    {
      filePath: { type: 'string', required: true, description: PEN_FILE_PATH_DESCRIPTION },
    },
    async (args, exec) => {
  ...
  register('pencil_mcp_get_screenshot', 'Render a .pen node (nodeId "document" = whole file) to an image returned directly as an attachment — a visual-fidelity spot check (colors, font rendering, alignment/spacing, layout positions). Screenshots are expensive: use sparingly and prefer the smallest meaningful node. Text and property content is verified with pencil_mcp_batch_get, not from the image.',
    ...
    async (args, exec) => {
      ...
      // The editor's get-screenshot IPC is a viewport capture and does not
      // reliably honor nodeId. When a live canvas owns the document, render a
      // node with its official export API instead of consulting a second,
      // potentially stale headless scene.
      if (nodeId !== 'document' && headless.canvasActive(exec)) {
        return exportNodeShot(filePath, nodeId, exec, { liveRequired: true })
      }
      ...
      if (nodeId !== 'document') {
        ...
        if (big) {
          // Active canvases returned through their live export path above;
          // this branch retries the CLI export for a large headless node.
          // scale 1 is already much clearer than the native thumbnail, while
          // larger scales make complex nodes time out more often.
          const exported = await exportNodeShot(filePath, nodeId, exec)
          if (exported) return exported
          // Export failure falls back to the native screenshot in headless
          // mode. A live-canvas failure was returned explicitly above.
        }
      }
  ...
  register('pencil_mcp_insert_image',
    'Insert a chat image (or a local image file) onto the .pen canvas using pen.dev\'s official image-fill: it writes the image into the `images/` directory next to the .pen and adds a Frame whose `fill` is `{type:"image", url, mode}`. Pass `latest` for the most recently observed conversation image, `recent:N` for the Nth most recent image (`recent:1` = latest), an exact chat attachment id, or a local image file path as `image`. Conversation aliases resolve the original image attachment even when an image-capable wrapper has rewritten that image to OCR/description text for a text-only model; a direct text-only Harness route cannot admit chat images. Target the .pen via `filePath` (defaults to the current .pen) and optionally set `parentId` / `width` / `height` / `x` / `y` / `mode` (fit|fill|stretch, default fit). Returns the .pen-relative image URL and the created node.',
    {
      image: { type: 'string', required: true, description: '`latest`, `recent:N`, an exact chat image attachment id, or a local image file path (workspace-relative or absolute). Conversation aliases select the original attachment, not OCR/description text produced by a vision wrapper.' },
      filePath: { type: 'string', description: PEN_FILE_PATH_DESCRIPTION + ' Defaults to the session current .pen.' },
      parentId: { type: 'string', description: 'Parent node id to insert into (default: top level / document).' },
      name: { type: 'string', description: 'Optional name for the inserted node.' },
      width: { type: 'number', description: 'Node width in px.' },
      height: { type: 'number', description: 'Node height in px.' },
      x: { type: 'number', description: 'Left position in px (absolute layout).' },
      y: { type: 'number', description: 'Top position in px (absolute layout).' },
      mode: { type: 'string', enum: ['fit', 'fill', 'stretch'], description: 'Image-fit mode (default fit).' },
    },
    async (args, exec) => {
      const workspace = workspaceForExec(exec)

      // 1. Resolve the target .pen file.
      let penFile
      if (args.filePath) {
        penFile = resolveWorkspacePath(workspace, String(args.filePath), { extension: '.pen', label: 'filePath' })
      } else {
        penFile = getSessionFile(sessionIdOf(exec))
        if (!penFile) {
          return { ok: false, text: 'pencil_mcp_insert_image: no current .pen file; pass filePath or open one first with pencil_mcp_open.' }
        }
      }

      // 2. Resolve image bytes + media type (chat attachment first, then file path).
      const session = exec && exec.agent && exec.agent.session
      let bytes
      let mediaType
      let via = 'file'
      const imageInput = String(args.image).trim()
      const ref = sessionImages && sessionImages.lookup(session, imageInput)
      if (ref && attachments && typeof attachments.readImage === 'function') {
        try {
          const stored = await attachments.readImage(ref, exec.signal)
          bytes = stored && stored.data
          mediaType = stored && stored.ref && stored.ref.mediaType
          via = 'chat-attachment'
        } catch (error) {
          return { ok: false, text: 'pencil_mcp_insert_image: failed to read chat attachment: ' + (error && error.message ? error.message : String(error)) }
        }
      }
      if (!bytes && /^(?:latest|recent:\d+)$/i.test(imageInput)) {
        return { ok: false, text: `pencil_mcp_insert_image: no conversation image matches "${imageInput}"; ask the user to attach the image again or pass a local file path.` }
      }
      if (!bytes) {
        const resolved = resolveWorkspacePath(workspace, imageInput, { label: 'image' })
        try {
          bytes = await fsp.readFile(resolved)
        } catch (error) {
          return { ok: false, text: 'pencil_mcp_insert_image: image is neither a known chat attachment id nor a readable file: ' + (error && error.message ? error.message : String(error)) }
        }
        mediaType = sniffImageMediaType(bytes) ?? extensionToMediaType(path.extname(resolved))
      }
      if (!bytes || bytes.length === 0) return { ok: false, text: 'pencil_mcp_insert_image: image is empty' }
      if (bytes.length > 20 * 1024 * 1024) return { ok: false, text: 'pencil_mcp_insert_image: image exceeds the 20 MiB limit' }
      mediaType = mediaType ?? sniffImageMediaType(bytes)
      mediaType = String(mediaType || '').toLowerCase()
      if (!IMAGE_MEDIA_TYPES.has(mediaType)) return { ok: false, text: 'pencil_mcp_insert_image: unsupported image format (png/jpeg/webp/gif only)' }

      const width = args.width === undefined || args.width === null ? 400 : Number(args.width)
      const height = args.height === undefined || args.height === null ? 300 : Number(args.height)
      if (!Number.isFinite(width) || width <= 0 || !Number.isFinite(height) || height <= 0) {
        return { ok: false, text: 'pencil_mcp_insert_image: width and height must be positive finite numbers' }
      }
      const mode = String(args.mode || 'fit')
      if (!['fit', 'fill', 'stretch'].includes(mode)) {
        return { ok: false, text: 'pencil_mcp_insert_image: mode must be fit, fill, or stretch' }
      }

      // 3. Write the image into images/ next to the .pen (pen.dev convention).
      const { absolute: imageFile, relativeUrl } = await writeImageAsset(path.dirname(penFile), bytes, mediaType)
      ...
      const node = {
        ...
        width,
        height,
        fill: { type: 'image', url: relativeUrl, mode },
      }
      if (args.x !== undefined && args.x !== null && Number.isFinite(Number(args.x))) node.x = Number(args.x)
      if (args.y !== undefined && args.y !== null && Number.isFinite(Number(args.y))) node.y = Number(args.y)
      const parentId = String(args.parentId || 'document')
      const input = `Insert(${JSON.stringify(parentId)}, ${JSON.stringify(node)})`
      // If the call rejects, preserve the asset: the transport may have lost
      // its response after applying the edit, and a committed node must not be
      // left with a broken image URL.
      const result = await runMcp('execute', { filePath: penFile, input }, { exec, signal: exec.signal })
      if (!result || !result.ok) {
        // A failed save leaves the edit live in memory, so its image must stay.
        // Definite tool/validation failures happen before mutation and are safe
        // to roll back by removing the newly written asset.
        const editSucceeded = result && /\bedit succeeded\b/i.test(String(result.text || ''))
        if (!editSucceeded) await fsp.unlink(imageFile).catch(() => {})
        return { ok: false, text: 'pencil_mcp_insert_image: insert failed: ' + (result && result.text ? result.text : 'execute returned no result') }
      }
      const summary = typeof result.text === 'string' && result.text.trim() ? result.text.trim() : ''
      return {
        ok: true,
        text: (summary ? summary + '\n' : '') +
          `pencil_mcp_insert_image: placed image at ${relativeUrl} (relative to ${path.basename(penFile)}); resolved via ${via}.`,
      }
    }, 60000)

  return legacyToolsEnabled ? 14 : 9
}

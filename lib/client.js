...
`
		...
		function useHarnessLanguage() {
			const [language, setLanguage] = React.useState(harnessLanguage)
			React.useEffect(() => {
				const sync = () => setLanguage(harnessLanguage())
				const observer = new MutationObserver(sync)
				observer.observe(document.documentElement, { attributes: true, attributeFilter: ['lang'] })
				window.addEventListener('languagechange', sync)
				return () => { observer.disconnect(); window.removeEventListener('languagechange', sync) }
			}, [])
			return language
		}
		...
		const LOCALIZED_CANVAS_LABELS = {
			zh: {
				...
				float: '浮窗', split: '分屏', switchToFloat: '切换为浮动窗口', switchToSplit: '切换为右侧分屏',
			},
			en: {
				...
				float: 'Float', split: 'Split', switchToFloat: 'Switch to floating window', switchToSplit: 'Switch to right split',
			},
		}
		...
		function PenCanvas(props) {
			...
			const language = useHarnessLanguage()

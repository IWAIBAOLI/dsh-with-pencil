...
.dsh-penhost-menu-item:hover { background: color-mix(in srgb, currentColor 8%, transparent); }
			...
			const exportCanvas = async (format) => {
				...
				try {
					...
					setExportResult({
						...
						message: (result.scope === 'selection' ? '已导出选区' : '已导出当前文档') + ' · ' + count + ' 个文件',
					})
				} catch (error) {
					setMenuError(error && error.message ? error.message : String(error))
				...
				React.createElement('div', { className: 'dsh-penhost-head', onPointerDown: startDrag },
					...
					React.createElement('div', { className: 'dsh-penhost-menu-wrap dsh-penhost-export-wrap', ref: exportMenuRef, onPointerDown: (event) => event.stopPropagation() },
						...
						menu === 'export' ? React.createElement('div', { className: 'dsh-penhost-menu', role: 'menu' },
							React.createElement('div', { className: 'dsh-penhost-menu-note' }, '有选区时导出选区，否则导出全部顶层元素。'),
							React.createElement('button', { className: 'dsh-penhost-menu-item', role: 'menuitem', disabled: !!exporting, onClick: () => { void exportCanvas('png') } }, exporting === 'png' ? '正在导出 PNG…' : '导出 PNG（2×）'),
							React.createElement('button', { className: 'dsh-penhost-menu-item', role: 'menuitem', disabled: !!exporting, onClick: () => { void exportCanvas('pdf') } }, exporting === 'pdf' ? '正在导出 PDF…' : '导出 PDF'),
							exportResult ? React.createElement(React.Fragment, null,
								React.createElement('div', { className: 'dsh-penhost-menu-sep' }),
								React.createElement('div', { className: 'dsh-penhost-menu-note', title: exportResult.directory }, exportResult.message + ' → ' + exportResult.directory),
								React.createElement('button', { className: 'dsh-penhost-menu-item', role: 'menuitem', onClick: () => { void revealExport() } }, '打开导出文件夹')) : null,
							menuError ? React.createElement('div', { className: 'dsh-penhost-menu-note' }, menuError) : null) : null),

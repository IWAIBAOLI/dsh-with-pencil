...
.dsh-penhost-settings-note { margin: 10px 0 0; padding-top: 8px; border-top: 1px solid var(--dsw-alias-border-l2, GrayText); font-size: 12px; line-height: 1.5; color: var(--dsw-alias-label-secondary, GrayText); }
...
`
		...
		const PENCIL_SETTINGS_LABELS = {
			zh: {
				...
				note: '注意：设置 → 模型 页面出现的「Pencil 设计插件（官方限制占位）」条目，是 DeepSeek Harness 设置边界限制所致（第三方插件的设置目前只能借模型供应商目录暴露，官方尚未开放插件自注册）。这不是本插件的问题；等官方开放后我们会撤掉它。',
			...
			en: {
				...
				note: 'Note: the "Pencil 设计插件（官方限制占位）" entry on the Models page is a placeholder forced by DeepSeek Harness\'s settings-seam limitation (third-party settings can only be exposed through the model-provider directory; plugin self-registration is not open yet). It is not a defect of this plugin and will be removed once the harness opens it up.',
					...
					React.createElement('div', { className: 'dsh-penhost-settings-row' },
						...
						React.createElement('select', {
							...
							React.createElement('option', { value: 'multimodal' }, labels.modeMultimodal))),
									...
									React.createElement('p', { className: 'dsh-penhost-settings-note' }, labels.note),
